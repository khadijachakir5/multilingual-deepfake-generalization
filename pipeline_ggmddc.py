...
"""
================================================================================
GGMDDC — Diagnostic Pipeline (SQ1–SQ4) — Controlled Single-Generator Baseline
================================================================================
Corpus      : GGMDDC (Purohit et al., APSIPA ASC 2024)
              80,000 utterances, 10 languages, 1 HiFi-GAN generator
Languages   : arabic, english, french, hindi, mandarin,
              portuguese, russian, sanskrit, spanish, vietnamese
Protocols   : SQ1 + SQ2 + LOLO (10 folds) + SQ4(LOLO)
              LOGO: NOT APPLICABLE (single generator by design)

Paper role  : Controlled baseline — isolates linguistic variability
              in the absence of generator-related confounds.
              Establishes baseline separability and cross-lingual
              language effects without generator interference.

Usage:
  python pipeline_ggmddc.py --root /path/to/GGMDDC
  python pipeline_ggmddc.py --features features_ggmddc.pkl

Output: results_ggmddc/tables/
  SQ1_representation_space.csv  — Hellinger + eta2 (language only)
  SQ2_invariance.csv            — MW + KW + BH-FDR
  SQ3_lolo_ggmddc.csv           — LOLO 10 folds x 9 families
  SQ4_shap_lolo.csv             — SHAP stratified TP/FP/FN/TN
================================================================================
"""
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="GGMDDC diagnostic pipeline — SQ1 to SQ4 (LOLO only)"
    )
    p.add_argument("--root",     default="",
                   help="Path to GGMDDC dataset root directory")
    p.add_argument("--output",   default="results_ggmddc",
                   help="Output directory (default: results_ggmddc/)")
    ...
    logger.info("=" * 60)
    logger.info("GGMDDC — Controlled Single-Generator Baseline")
    logger.info("  10 languages | 1 generator (HiFi-GAN) | LOLO only")
    logger.info("=" * 60)
    ...
    if cfg.features_file and Path(cfg.features_file).exists():
        ...
        audio_files = scan_ggmddc(cfg)
        if not audio_files:
            logger.error("No audio files found. Check --root.")
            return
    ...
    # ── SQ1: Representation Space Analysis ───────────────────────────────────
    # Hellinger distance + eta2_lang (eta2_gen NOT applicable: 1 generator)
    logger.info("\n[SQ1] Representation Space Analysis")
    df_sq1 = sq1(df, families, cfg)

    # ── SQ2: Cross-Lingual Stability and Separability ────────────────────────
    # Mann-Whitney U + Kruskal-Wallis H, BH-FDR corrected
    logger.info("\n[SQ2] Cross-Lingual Stability and Separability")
    df_sq2 = sq2(df, families, cfg)

    # ── SQ3: LOLO (Leave-One-Language-Out) ───────────────────────────────────
    # 10 folds (one per language), RF + LR classifiers
    # LOGO not applicable: single generator by design
    logger.info("\n[SQ3-LOLO] Cross-Lingual Robustness — 10 folds")
    df_lolo = sq3_lolo(df, families, cfg)
    df_delta = compute_delta(df_lolo, None, cfg)

    # ── SQ4: SHAP Mechanism Attribution (LOLO) ───────────────────────────────
    # Stratified by TP/FP/FN/TN — identifies German over-activation
    logger.info("\n[SQ4-LOLO] SHAP Mechanism Attribution")
    sq4_shap(df, families, cfg, protocol="lolo")

    logger.info("\n" + "=" * 60)
    logger.info("GGMDDC PIPELINE COMPLETE")
    logger.info("=" * 60)
